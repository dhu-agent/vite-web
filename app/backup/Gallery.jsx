...
import ListPage from "../Components/Gallery/ListPage.jsx";
import "./Gallery.css";
...
const Gallery = () => {
  ...
  //       }
  //    else {
  const { agents, setAgents, tools, setTools, models, setModels } =
        ...
        const agentsWithTools = await Promise.all(
          agentsRes.data.map(async (agent) => {
            const toolsRes = await axios.get(
              `${API_BASE}/v1/agent_tools/get/${agent.agent_id}`
            );
            return {
              id: agent.agent_id,
              name: agent.name,
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      if (activeTab === "Agents") {
        ...
        if (currentItem.id) {
          ...
          const response = await axios.put(
            `${API_BASE}/v1/llm/update/${currentItem.id}`,
            modelPayload
          );
      ...
      message.error(
        `Failed to ${currentItem.id ? "update" : "create"} ${activeTab}`
      );
      console.error(error);
    } finally {
      setLoading(false);
              ...
              {modalFields.includes("inputs") && (
                <label>
                  ...
                  <input
                    type="text"
                    name="inputs"
                    defaultValue={currentItem.inputs}
                    required
                  />
                </label>
              )}
              {modalFields.includes("output_type") && (
                <label>
                  ...
                  <input
                    type="text"
                    name="output_type"
                    defaultValue={currentItem.output_type}
                    required
                  />
                </label>
              )}
        ...
        <div className="tm-modal-overlay">
          <div className="tm-modal">
            ...
            <div className="tool-list">
              {tools.map((tool) => (
                <div
                  key={tool.id}
                  className={`tool-item ${
                    currentItem?.tools?.includes(tool.id) ? "selected" : ""
                  }`}
                  onClick={() => {
            ...
            <div className="tm-modal-actions">
              <button onClick={() => setShowToolModal(false)}>完成</button>
            </div>
          </div>
        </div>
      )}
              ...
              {models.map((model) => (
                <div
                  ...
                  className={`model-item ${
                    currentItem?.model === model.id ? "selected" : ""
                  }`}
                  onClick={() => {
